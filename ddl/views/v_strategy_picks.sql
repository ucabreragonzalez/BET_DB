CREATE OR REPLACE VIEW v_strategy_picks AS
    SELECT 
        ...
        if(r.full_time_home_team_goals is null,null,
			case
				when sr.forecast = '+2.5 goals' and r.full_time_home_team_goals + r.full_time_away_team_goals > 2.5 then 'W'
				when sr.forecast = '-2.5 goals' and r.full_time_home_team_goals + r.full_time_away_team_goals < 2.5 then 'W'
				else 'L'
			end
        ) as strategy_result,
